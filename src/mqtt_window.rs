        ...
        .show(ctx, |ui| {
            ui.columns(2, |columns| {
                columns[0].vertical(|ui| {
                    // Top section with broker config and discovery
                    ui.group(|ui| {
                ...
                columns[1].group(|ui| {
                    if mqtt_cfg_window.broker_status().reachable() && !mqtt_cfg_window.selected_topics().is_empty() {
                        ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
                            if ui.add_sized([120.0, 30.0], egui::Button::new(RichText::new("Connect").strong())).clicked() {
...
fn show_subscribed_topics(ui: &mut Ui, mqtt_cfg_window: &mut MqttConfigWindow) {
    ...
    let label_txt = if subscribed_topics == 0 {
        "Select topics to subscribe to before connecting".into()
    } else {
        format!("Subscribed Topics ({subscribed_topics}):")
    };
    ui.label(label_txt);
...
fn show_discovered_topics_list(ui: &mut Ui, mqtt_cfg_window: &mut MqttConfigWindow, topics: Vec<String>) {
...
fn show_discovered_topics_section(ui: &mut Ui, mqtt_cfg_window: &mut MqttConfigWindow) {
    ...
    if discovered_sys_topics > 0 {
        ui.collapsing(format!("Broker sys topics ({discovered_sys_topics})"), |ui| {
            ui.separator();
            show_discovered_topics_list(ui, mqtt_cfg_window, mqtt_cfg_window.discovered_sys_topics_sorted());
        });
    }
}
