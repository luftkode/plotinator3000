...
use plotinator_log_if::log::GitMetadata;
use serde::{Deserialize, Serialize};
use serde_big_array::BigArray;

#[derive(Debug, PartialEq, Deserialize, Serialize, Clone, Copy)]
pub(crate) struct PidLogHeaderV3 {
    #[serde(with = "BigArray")]
...
impl BuildMbedLogHeaderV2<MbedConfigV2> for PidLogHeaderV3 {
    fn new(
        unique_description: UniqueDescriptionData,
        version: u16,
        project_version: ProjectVersionData,
...
impl GitMetadata for PidLogHeaderV3 {
...
impl MbedMotorControlLogHeader for PidLogHeaderV3 {
    const VERSION: u16 = 2;
...
impl fmt::Display for PidLogHeaderV3 {
...
mod tests {
    ...
    fn test_deserialize() -> TestResult {
        let mut file = fs::File::open(mbed_pid_v3())?;
        let mut buffer = vec![0; 500];
        file.read_exact(&mut buffer)?;
        let (pid_log_header, bytes_read) = PidLogHeaderV3::from_reader(&mut buffer.as_slice())?;
        ...
        assert_eq!(pid_log_header.git_short_sha(), Some("303ca3c".into()));
        ...
        assert_eq!(pid_log_header.mbed_config().t_fan_on(), 68);
        assert_eq!(pid_log_header.mbed_config().t_fan_off(), 67);
        assert_eq!(pid_log_header.mbed_config().rpm_standby(), 3600);
        assert_eq!(pid_log_header.mbed_config().rpm_running(), 6300);
        assert_eq!(pid_log_header.mbed_config().time_in_idle(), 15);
        assert_eq!(pid_log_header.mbed_config().time_shutdown(), 5);
        assert_eq!(pid_log_header.mbed_config().time_wait_for_cap(), 300);
        assert_eq!(pid_log_header.mbed_config().vbat_ready(), 12.8);
        assert_eq!(pid_log_header.mbed_config().servo_max(), 1620);
