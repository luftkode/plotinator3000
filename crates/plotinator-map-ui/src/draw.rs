...
pub(crate) fn draw_path_inner(
    ...
    for window in screen_points.windows(2) {
        ...
        let altitude = window[0].1.altitude.first().map_or(0.0, |a| a.inner_raw());
...
pub(crate) fn draw_heading_arrow(
    ...
    let Some((origin, vec)) = calculate_arrow_vector(center, geo_point, speed_range) else {
        return;
    };
    ...
    painter.arrow(origin, vec, outline_stroke);
}

#[inline]
pub(crate) fn calculate_arrow_vector(
    center: Pos2,
    geo_point: &GeoPoint,
    speed_range: (f64, f64),
) -> Option<(Pos2, Vec2)> {
